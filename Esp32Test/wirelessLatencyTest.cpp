...
volatile bool startSampling = false; // can only be changed from python program
...
void sendTask(void* pvParameters) {
  while (true) { //runs constantely
    ...
    bufferReady = false; //can start samling again
    startSampling = false; // reseting startSampling after one packet is sent
...
void loop() {
  if (Serial.available()) { //incoming data from computer
    char start = Serial.read();
    if (start == 's') { // if data is 's' from computer start one cycle
    portENTER_CRITICAL(&timerMux);
    startSampling = true;
    bufferIndex = 0; 
    bufferReady = false; 
